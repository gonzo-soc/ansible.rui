...
main() {
  ...
  local -r __dhcpd="$ROOT_DIR_ROLL_UP_IT/libs/lnx_centos07/utils/dhcp.sh"

  ${__dhcpd} -c "tmp/common_opts_dhcp_srv.txt"
  $ROOT_DIR_ROLL_UP_IT/libs/lnx_centos07/utils/dhcp.sh -s "tmp/subnet_dhcp_srv.txt"
  # deploy the configuration
  # i$ROOT_DIR_ROLL_UP_IT/libs/lnx_centos07/utils/dhcp.sh -d
